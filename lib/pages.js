...
function layout({ title, description, active, locale, path, body, extraNav, footerLeft, footerRight, after, image, head }) {
  const t = (key, vars) => i18n.t(locale, key, vars);
...
<head>
  ...
  <meta property="og:site_name" content="Glimty">
...
// Tiles crop a 4:3 window out of a 2.6:1 photo, so the rendered image is about twice the tile width.
const TILE_SIZES = '(max-width: 860px) 95vw, (max-width: 1040px) 62vw, 46vw';

function smallImage(src) {
  return String(src).replace('/images/gifts/', '/images/gifts/sm/');
}

function tileSources(src) {
  return `src="${escapeHtml(smallImage(src))}" srcset="${escapeHtml(smallImage(src))} 720w, ${escapeHtml(src)} 1100w" sizes="${TILE_SIZES}"`;
}
...
function giftCard(item, locale) {
  ...
  return `<a class="gift${status === 'out' ? ' is-out' : ''}" href="/gift/${escapeHtml(gift.id)}?lang=${locale}">
    <div class="gift-media">
      <img ${tileSources(gift.image)} alt="" loading="lazy" decoding="async" width="1100" height="424"${focusStyle(item)}>
      ${badge}
    </div>
    <div class="gift-body">
...
// Cards for a list of gift ids, used by the recently-viewed row.
function giftCards(ids, locale) {
  return ids
    .map((id) => inventory.getGift(id))
    .filter(Boolean)
    .slice(0, 4)
    .map((item) => giftCard(item, locale))
    .join('');
}

const recentSection = (locale, currentId) => `
    <section class="section recent" data-recent${currentId ? ` data-current="${escapeHtml(currentId)}"` : ''} hidden>
      <h2>${escapeHtml(i18n.t(locale, 'recent_title'))}</h2>
      <div class="shop-grid" data-recent-grid></div>
    </section>
    <script src="/js/recent.js" defer></script>`;
...
function categoryTile(cat, locale) {
  ...
  return `<a class="cat" href="/shop/${escapeHtml(cat.id)}?lang=${locale}">
    ${cover ? `<img ${tileSources(cover.image)} alt="" loading="lazy" decoding="async" width="1100" height="424"${focusStyle(cover)}>` : ''}
  ...
  return layout({
    ...
    <section class="hero">
      ...
      <div class="hero-photos" aria-hidden="true">
        ${heroItems.map((item) => `<img src="${escapeHtml(item.image)}" srcset="${escapeHtml(smallImage(item.image))} 720w, ${escapeHtml(item.image)} 1100w" sizes="(max-width: 860px) 72vw, 50vw" alt="" width="1100" height="424">`).join('')}
      </div>
    </section>
...
// 0 when the gift does not match; higher when the words hit the name, brand or category
// rather than only the blurb or colours.
function searchScore(item, locale, needle) {
  const gift = inventory.publicGift(item, locale);
  const words = needle.split(/\s+/).filter(Boolean);
  const strong = [gift.name, gift.brand, i18n.categoryCopy(item.category, locale).label].join(' ').toLowerCase();
  const weak = [gift.blurb, gift.sku].concat(gift.colors).join(' ').toLowerCase();
  if (!words.every((word) => strong.includes(word) || weak.includes(word))) return 0;
  return 1 + words.filter((word) => strong.includes(word)).length;
}
...
function shopPage(categoryId, locale, sort, search, budget) {
  ...
  const scores = new Map(q ? inCategory.map((item) => [item, searchScore(item, locale, q.toLowerCase())]) : []);
  const searched = q
    ? inCategory.filter((item) => scores.get(item) > 0).sort((a, b) => scores.get(b) - scores.get(a))
    : inCategory;
...
function productPage(id, locale, origin = '') {
  ...
  const structured = {
    ...
    description,
    sku: gift.sku,
    image: `${origin}${gift.image}`,
    brand: { '@type': 'Brand', name: gift.brand },
    offers: {
      '@type': 'Offer',
      price: gift.price,
      priceCurrency: 'USD',
      availability: `https://schema.org/${status === 'out' ? 'OutOfStock' : status === 'low' ? 'LimitedAvailability' : 'InStock'}`,
      url: `${origin}/gift/${item.id}`
    }
  };
  ...
  return layout({
    ...
    <article class="product">
      ...
      <img class="product-photo" src="${escapeHtml(gift.image)}" fetchpriority="high" alt="${escapeHtml(gift.name)}" width="1100" height="424">
...
module.exports = { giftCards, shopPage, productPage, giftCard, homePage, assistantPage, notFoundPage, langSwitch };
