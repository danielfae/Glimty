...
function audienceSwitch(locale, active) {
  ...
  return `<div class="audience-switch" role="group" aria-label="${escapeHtml(t('nav_audience_label'))}">
    <a class="${active === 'business' ? '' : 'is-on'}" href="/?lang=${locale}">${escapeHtml(t('nav_private'))}</a>
    <a class="${active === 'business' ? 'is-on' : ''}" href="/business?lang=${locale}">${escapeHtml(t('nav_business'))}</a>
  </div>`;
}
