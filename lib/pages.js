...
function shopPage(categoryId, locale, sort, search) {
  const t = (key, vars) => i18n.t(locale, key, vars);
  ...
  const base = q ? inCategory.filter((item) => matchesQuery(item, locale, q.toLowerCase())) : inCategory;
  ...
  const query = (sortValue, keepSearch = true) => (
    `?lang=${locale}${sortValue ? `&sort=${sortValue}` : ''}${q && keepSearch ? `&q=${encodeURIComponent(q)}` : ''}`
  );
  ...
  const empty = `<div class="shop-empty">
      <h2>${escapeHtml(t('search_none_title', { q }))}</h2>
